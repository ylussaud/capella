...
public class DiagramTransitionRunnable extends AbstractProcessingCommands<DDiagram> {
  ...
  protected IStatus proceedDiagram(final DRepresentation diagram, final IProgressMonitor monitor) {
    ...
    if (!(diagram instanceof DSemanticDecorator)) {
      return new Status(IStatus.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), "Invalid source diagram");
    }
    ...
    if ((sourceSemantic == null) || sourceSemantic.eIsProxy()) {
      return new Status(IStatus.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), "Diagram with invalid semantic target");
    }

    // Retrieve the current session
    final Session session = DiagramHelper.getService().getSession(diagram);
    if (session == null) {
      return new Status(IStatus.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), "Cannot retrieve session from the given diagram");
    }
    ...
    if ((description == null) || description.eIsProxy()) {
      return new Status(IStatus.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), "Invalid source diagram (maybe a missing viewpoint)");
    }
    ...
    if (targetDiagram == null) {
      return new Status(IStatus.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), "Invalid target diagram");
    }
    return new Status(IStatus.OK, FrameworkUtil.getBundle(getClass()).getSymbolicName(), NLS.bind("Diagram initialized - {0}", diagramName));
  }
  ...
  protected IStatus retrieveTargetDiagram(DRepresentation diagram, Session session, EObject sourceSemantic,
    ...
    if (targetDiagram == null) {
      return (new Status(IStatus.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), "Cannot create the target diagram"));
    }
