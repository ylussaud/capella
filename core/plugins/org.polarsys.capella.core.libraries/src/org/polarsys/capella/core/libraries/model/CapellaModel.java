...
public class CapellaModel extends AbstractCapellaModel implements IModel.Edit {
  ...
  public boolean isActive(IModel library) {
    Preferences activeLibraryPrefInfos = ConfigurationScope.INSTANCE.getNode(FrameworkUtil.getBundle(getClass()).getSymbolicName()).node(ACTIVE_STATE_PREF_ID);
  ...
  public void setActive(IModel library, boolean activeState) {
    final Preferences activeLibraryPrefInfos = ConfigurationScope.INSTANCE.getNode(FrameworkUtil.getBundle(getClass()).getSymbolicName()).node(ACTIVE_STATE_PREF_ID);
    Preferences libraryContextPref = activeLibraryPrefInfos.node(this.getIdentifier().getName());
