...
public abstract class AbstractToolWrapper {
  ...
  final public IStatus checkArguments() {
    boolean ret = true;
    MultiStatus result = new MultiStatus(FrameworkUtil.getBundle(getClass()).getSymbolicName(), Status.OK, "isArgumentsAreSet", null);
    ...
    while (it.hasNext() && ret) {
      ...
      if (((currentType.equals(ArgumentType.COLLECTION) && _arguments.get(currentType) == null))
        ...
        result.add(new Status(Status.OK, FrameworkUtil.getBundle(getClass()).getSymbolicName(), currentType + " is ok"));
        return result;
      }
      
      if (!_arguments.containsKey(currentType) || (_arguments.get(currentType) == null)) {
        result.add(new Status(Status.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), currentType + " is not set or null"));
        ...
        if (ret) {
          ...
          result.add(new Status(Status.ERROR, FrameworkUtil.getBundle(getClass()).getSymbolicName(), currentType + " is not value"));
        }
      }
    }
    return result;
